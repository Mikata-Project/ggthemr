plot_colours <- function(colour_vector, background_colour = 'white') {
  ...
  ggplot(colour_df, aes(factor(1), Colours, fill=Colours)) + geom_tile() + 
    scale_fill_manual(values=levels(colour_df$Colours), guide='none') +
    geom_text(aes(label=Colours), colour='white', vjust=1.1, fontface='bold') +  
    geom_text(aes(label=Colours), colour='black', vjust=-0.1, fontface='bold') + 
